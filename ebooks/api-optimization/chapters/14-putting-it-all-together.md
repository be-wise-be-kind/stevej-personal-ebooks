...
2. **Async processing (Chapter 8)**: If work can be deferred and acknowledged immediately.
...
4. **Backpressure (Chapter 8)**: Slow producers when consumers are overwhelmed.
...
2. **Async email delivery (Chapter 8)**: Changed from synchronous email sending to fire-and-forget pattern with a message queue. Emails now sent by a background worker with retry logic.
